...
«Серьезный» (Major) – дефект приводящий к некоректной работе части функциональности, недоступности опций отдельной функции, но не блокирующий тестирование в целом . Также это может быть блокирующая ошибка для отдельной функции, не оказывающей влияния на работу всей системы или ветви её функционала;
...
«Уделить повышенное внимание» ( high ) – нет необходимости в сверхсрочном исправлении дефекта, но устранение ошибки должно быть осуществлено непосредственно.
«Обычный» (Normal ) – дефолтный  приоритет при регистрации дефектов.
«Низкий» (Low) – дефекты исправляются только после устранения всех других ошибок в программе.
Улучшение (Enchancement) – предложения тестовой группы по улучшению качества интерфейса (внедряется только после валидации заказчиком).
...
| **Summary**  | Отсутствие на странице Каталога некоторых изображений продукции магазина и логотипов разделов  |
| ------------- | ------------- |
