 ...
 ***DevTools***
 Отсутствие ошибок в Console.    |  ⛔           |    по указанным путям отсутствуют изображения       |
...
- При просмотре кода была обнаружена Форма обратной связи, но она была скрыта. При проверке выяснилось, что форма не отправляет запрос на сервер.
